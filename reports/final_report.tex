...
\title{Replication of ``Can ChatGPT Forecast Stock Price Movements?'' \cite{LopezLiraTang2023}}
\author{}
\date{\today}
...
The replication pipeline follows the same general structure as the methodology described in \cite{LopezLiraTang2023}. The analysis combines financial news headlines with stock return data and uses a large language model to interpret the news content.
...
The classification step follows the prompt described in the paper \cite{LopezLiraTang2023}. Each headline is evaluated by the model -- in this replication, GPT3.5-Turbo is used because of its cut-off date of Sept. 2021 to avoid lookahead bias-- and assigned one of three possible labels. A headline can imply that the firm's stock price is likely to increase (YES), decrease (NO), or that the headline contains no clear directional information (UNKNOWN). These outputs are aggregated at the firm-day level to produce a daily sentiment score for each firm.
...
While the replication pipeline closely follows the structure described in the paper \cite{LopezLiraTang2023}, several important differences arise due to data availability and model constraints.
...
\begin{figure}[H]
\centering
\safefigureorplaceholder[width=.9\textwidth]{../_output/portfolio_size_diagnostics_all.png}
...
\begin{table}[H]
\centering
\caption{Table 1 from the original paper \cite{LopezLiraTang2023}}
...
The replication broadly reproduces the qualitative pattern reported in the paper. The long-short portfolio generates positive drift returns and produces a Sharpe ratio that remains economically meaningful. However, the magnitude of the Sharpe ratio is lower than in the original study \cite{LopezLiraTang2023}. This difference is consistent with the smaller portfolio sizes and the higher share of neutral classifications produced by the model.
...
\begin{figure}[H]
\centering
\safefigureorplaceholder[width=.9\textwidth]{../_output/figure5_paper_sample.png}
\caption{Replication of Figure 5}
\label{fig:rep_fig5}
\end{figure}

The cumulative return profile of the strategy remains positive over the sample period. Although the growth rate is lower than that reported in the original paper \cite{LopezLiraTang2023}, the general pattern of sustained positive returns is preserved.
