...
def make_requests_jsonl(
    df: pd.DataFrame, model: str, requests_output_dir: Path, id_row_json_output_dir: Path, starting_row_idx: int=0) -> dict[str, dict[str, str]]:
...
def main():
    ...
    if OPENAI_BATCH_SIZE > MAX_BATCH_SIZE:
        raise EnvironmentError(f"OPENAI_BATCH_SIZE of {OPENAI_BATCH_SIZE} exceeds the maximum allowed batch size of {MAX_BATCH_SIZE}.")
    ...
    if size <= OPENAI_BATCH_SIZE:
        print(f"Input size {size:,} is less than or equal to batch size {OPENAI_BATCH_SIZE:,}. No batching required.")
        chunk_size = size
        make_requests_jsonl(
        ...
        while to_process > 0:
            ...
            make_requests_jsonl(
                chunk_df, model=OPENAI_MODEL,
                requests_output_dir=REQUESTS_JSONL_BASE.with_suffix(f".{json_idx}.jsonl"),
                id_row_json_output_dir=ID_ROW_JSON_BASE.with_suffix(f".{json_idx}.json"),
                starting_row_idx=chunk[0])
            
            prev_chunk_end += min(chunk_size, size+1)
            to_process -= chunk_size
            json_idx += 1
            print(f"Processed chunk {json_idx-1}, {max(to_process, 0):,} rows remaining")
